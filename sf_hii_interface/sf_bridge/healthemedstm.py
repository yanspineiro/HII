...
class HealtheMed_STM():
    def __init__(self):
        pass

    @staticmethod
    def send_quote_healthemed_SMT(endpoint, dict_values, user_id, plan_id):
    ...
    @staticmethod
    def send_enrollment_heathMed(host, dict_values, user_id, plan_id, quote_id, access_token, xml_quote, stm_questions):
        ...
        if dict_values.get('Plan_Size__c') == 'Individual + Spouse':
            SubElement(root, 'Plan_Type').text = 'Member+Spouse'
        if dict_values.get('Plan_Size__c') == 'Individual + Child':
            SubElement(root, 'Plan_Type').text = 'Member+Child'
        ...
        if str(dict_values.get('Product_Name')).__contains__('11 months'):
            SubElement(root, 'Duration_Coverage').text = '11'
        ...
        xml_value = '<?xml version="1.0" encoding="iso-8859-1"?>' + tostring(root)
        xml_value.replace('<Parent_Email />', '<Parent_Email>' + dict_values.get('Contact_Email__c') + '</Parent_Email>')
        xml_value.replace('<Email />', '<Email>' + dict_values.get('Contact_Email__c') + '</Email>')
    ...
    @staticmethod
    def get_question_healthemed_SMT(endpoint, quote_id, user_id, add_ons_list):
